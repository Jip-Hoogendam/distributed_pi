...
void main() {
    ...
    float x_offset = noise(st / 1000.0 + 900.0 + x_motion);
    float y_offset = noise(st / 1000.0 + 100.0 + y_motion);
    
    vec3 color = vec3(0.0);

    // Plot a line
    float pct = plot(vec2(x_offset, y_offset));
    color = (1.0-pct)*color+pct*vec3(x_offset * 100.0,1.0,y_offset * 100.0);
